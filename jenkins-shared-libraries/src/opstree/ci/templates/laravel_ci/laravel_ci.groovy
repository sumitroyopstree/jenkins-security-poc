package opstree.ci.templates.laravel_ci

import opstree.common.*
import opstree.laravel.*
        ...
        try {
            ...
            stage('Pre-Build Checks') {
                ...
                if (get_params_value(enableOverride, step_params, 'dependency_check') != null && get_params_value(enableOverride, step_params, 'dependency_check').toBoolean()) {
                    tasks['OWASPCodeDependencyScanning'] = {
            ...
            if (get_params_value(enableOverride, step_params, 'perform_code_build') != null && get_params_value(enableOverride, step_params, 'perform_code_build').toBoolean()) {
                stage('Build Artifact') {
                    build.build_factory(
                        ...
                        repo_url_type:        "${get_params_value(enableOverride, step_params, 'repo_url_type')}",
                        source_code_path:     "${get_params_value(enableOverride, step_params, 'source_code_path')}",
                        php_version:          "${get_params_value(enableOverride, step_params, 'php_version')}",
            ...
            if (get_params_value(enableOverride, step_params, 'perform_code_coverage') != null && get_params_value(enableOverride, step_params, 'perform_code_coverage').toBoolean()) {
                stage('Code Coverage') {
                    codecoverage.code_coverage_factory(
                        repo_url:              "${repo_url}",
                        repo_url_type:         "${get_params_value(enableOverride, step_params, 'repo_url_type')}",
                        source_code_path:      "${get_params_value(enableOverride, step_params, 'source_code_path')}",
                        php_version:           "${get_params_value(enableOverride, step_params, 'php_version')}",
                        perform_code_coverage: "${get_params_value(enableOverride, step_params, 'perform_code_coverage')}"
                    )
                }
            } else {
                echo 'Skipping Code Coverage stage as it is disabled.'
            }
            ...
            // Trigger CD pipeline on SUCCESS or UNSTABLE
